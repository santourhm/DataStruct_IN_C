...
hashvset_t hashvset_new(int n, unsigned int (*fhachage)(void * ),
    ...
    table->data = calloc(n, sizeof(*(table->data)));
    if (table->data == NULL) {
        free(table);
        return NULL;
    }
...
int hashvset_new_length(int sens, hashvset_t table) {
    ...
    for (int i = 0; i < table->size; i++) {
        if (table->data[i].busy == OCCUPED) {
            unsigned int slot = table->data[i].hashcode % new_size;
...
int hashvset_put(void* key, hashvset_t table) {
    ...
    // Redimensionner la table si le facteur de charge dépasse 0.7
    if (facteur_charge > 0.7) {
        if (!(hashvset_new_length(1, table))) {
            return -1;  // Échec du redimensionnement
        }
    }
    ...
    while (i < table->size) {
        ...
        // Si la case est libre (EMPTY ou FREE), insérer la clé
        if (table->data[slot].busy == EMPTY || table->data[slot].busy == FREE) {
            ...
            table->total_number++;  // Incrémenter le nombre total d'éléments
            return 1;
        }

        // Si la clé existe déjà, ne pas l'ajouter
    ...
    return 0;  // Échec de l'insertion (table pleine)
}
